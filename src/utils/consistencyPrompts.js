// Default consistency check prompt for legal case analysis
...
export const getActivePrompt = () => {
  const customPrompt = localStorage.getItem('consistency-prompt');
  return customPrompt || DEFAULT_CONSISTENCY_PROMPT;
};

// Save custom prompt to localStorage
export const saveCustomPrompt = (prompt) => {
  if (prompt && prompt.trim()) {
    localStorage.setItem('consistency-prompt', prompt);
    return true;
  }
  return false;
};
...
export const resetToDefaultPrompt = () => {
  localStorage.removeItem('consistency-prompt');
};

// Validate that a prompt contains the necessary structure
export const validatePrompt = (prompt) => {
  ...
  if (missingElements.length > 0) {
    return {
      valid: false,
      message: `Prompt missing required elements: ${missingElements.join(', ')}`
    };
  }
